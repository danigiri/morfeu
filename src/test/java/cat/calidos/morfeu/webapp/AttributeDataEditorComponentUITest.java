...
public void testBooleanAttributeTrue() {
	...
	assertAll("check default as true", () -> assertNotNull(bool),
...
public void testColorValidation() {
	...
	assertAll("check no validation message at first", () -> assertNotNull(color),
			() -> assertEquals("ff1100", color.value(),
					"attribute should have the right value at first"),
			() -> assertTrue(color.isEditable(), "attribute should be editable"),
			() -> assertTrue(color.validates(),
					"attribute should validate at first and show no warning"));
...
public void testSelectDropdown() {
	...
	assertAll("check no validation message at first", () -> assertNotNull(list),
			() -> assertEquals("A0", list.value(),
					"attribute should have the right value at first"),
			() -> assertTrue(list.isList(), "attribute should be a (dropdown) list"),
			() -> assertTrue(list.isEditable(), "attribute should be editable"),
			() -> assertTrue(list.validates(),
					"attribute should validate at first and show no warning"));
...
public void locatorDropdownTest() {
	...
	UIAttributeData list = new UIAttributeData();
	assertAll("check basics at first", () -> assertNotNull(list),
			() -> assertEquals("V0", list.value(),
					"locator attribute should have the right value at first"),
			...
			() -> assertTrue(list.validates(),
					"locator attribute should validate at first and show no warning"));

	List<String> possibleValues = list.possibleValues();
	assertAll("checking locator possible values", () -> assertNotNull(possibleValues),
