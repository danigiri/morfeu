...
public String name() {
	...
	name = name.endsWith(":") ? name.substring(0, name.length() - 1) : name; // the UI has a colon
																				// after the name
	name = name.endsWith("*") ? name.substring(0, name.length() - 1) : name; // if the attribute is
																				// mandatory
...
public String value() {
	return isEditable() ? element.$(ATTRIBUTE_DATA_VALUE).getValue()
			: element.$(ATTRIBUTE_DATA_VALUE).innerText();
}
...
public String validationWarning() {
	...
	if (validates()) {
		throw new IllegalStateException(
				"Cannot get a validation warning of a correct attribute, sorry");
	}
...
public UIAttributeData tabIntoEnterText(String value) {
	...
	for (int i = 0; i < attributeIndex; i++) { // it seems that we have the cursor on the first data
												// field for some reason
		pressKey("\t");
	}

	// this only works if called in order, selenide is not happy about the setValue so we're hacking
	// it
	pressKey(value);
...
public UIAttributeData toggle() {
	...
	if (!isEditable()) {
		throw new UnsupportedOperationException(
				"Cannot toggle the value of a non-editable boolean field");
	}
...
public UIAttributeData select(String value) {
	...
	if (!isEditable()) {
		throw new UnsupportedOperationException(
				"Cannot toggle the value of a non-editable dropdown field");
	}
...
public UIAttributeData clickOnCreate() {
	...
	if (!isNotPresent()) {
		throw new UnsupportedOperationException(
				"Cannot create an attribute unless it's not present");
	}
	if (!parent.isPresent()) {
	...
	return parent.get().attribute(this.name()); // and we now have new element, so we recreate
												// ourselves
