...
/**
 * Simple controller module that pongs back to the server the client-side event that was sent to the
 * backend
 * 
...
@Provides @IntoMap @Named("GET") @StringKey("/events/([^/\\?]+)")
public static BiFunction<List<String>, Map<String, String>, String> event() {
	return (pathElems,
			params) -> "{\"result\":\"" + pathElems.get(1) + "\", \"desc\":\""
					+ paramsToString(params) + "\"}";
}


@Provides @IntoMap @Named("Content-Type") @StringKey("/events/([^/\\?]+)")
