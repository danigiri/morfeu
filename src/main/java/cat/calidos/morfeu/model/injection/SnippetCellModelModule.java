...
public static URI modelURI(@Named("FilteredModelURI") URI uri) throws ConfigurationException {
	...
	if (queryIndex == -1) {
		throw new ConfigurationException(
				"Filtered model uri did not have '?filter=' (" + modelURI + ")");
	}
	...
	try {
		...
		throw new ConfigurationException("Problem when creating effective model URI for snippet",
				e);
	}
...
// return the uri of the snippet model so we can link it to the snippet content, at the moment we
// remove filter
@Produces @Named("CellModelFilter")
public static URI cellModelFilter(@Named("FilteredModelURI") URI uri)
	...
	try {
		...
		throw new ConfigurationException("Problem when creating the cell model filter for snippet",
				e);
	}
