...
public static Composite<Cell> produceContent(	@Named("ContentURI") URI uri,
	...
	Cell cell = DaggerCellComponent
			...
			.build()
			.createCell();
	if (!cellModelFilter.isPresent()) { // we have a root node which is this cell, which is an empty
										// root cell
...
// in this case, if we don't have a filter we return the whole document as, the cell structure in
// this case will contain
// an empty cell at the root to hold everything together
// If we have a filter, we are returning directly the first node as we do not have an empty cell
// node at the root
...
@Produces
public static Schema produceSchema(	@Named("FetchableModelURI") URI u,
									SchemaFactory sf,
	...
	try {
		...
		throw new ParsingException("Problem when reading the model schema", u.toString(), e);
	}
