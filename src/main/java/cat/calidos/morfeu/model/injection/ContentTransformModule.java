...
/**
 * Given in memory content (the model and cells) of a document, serialise to a specific format given
 * the URI extension or the supplied filters
 * 
...
public static String effectiveContent(	@Named("DestinationContentURI") URI uri,
	...
	try {
		...
		throw new TransformException("Problem when getting effective content to save '" + uri + "'",
				e);
	}
...
public static String filterContent(	@Named("ValuesForTemplate") Map<String, Object> values,
									@Nullable @Named("Filters") String filters,
									@Named("DestinationContentURI") URI uri)
		throws TransformException {
	...
	try {
		...
		String message = "Problem when filtering effective content to save '" + uri + "' with '"
				+ filters + "'";
		log.error(message);
		throw new TransformException(message, e);
