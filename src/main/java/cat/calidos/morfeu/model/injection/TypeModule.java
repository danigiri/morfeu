...
public static Type emptyType(	@Named("EffectiveURI") URI uri,
								String defaultName) {
...
URI uri(@Nullable XSType xsType,
		@Nullable URI uri) {

	URI u = uri; // if not overwritten by our caller (when uri!=null, we try to extract it from the
					// typee
	if (u == null) {
		try {
