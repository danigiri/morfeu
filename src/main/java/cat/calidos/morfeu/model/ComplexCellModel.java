...
/**
 * A Cell Model that has attributes (which cannot be repeated), order is considered important so
 * using list and not set
 * 
...
public ComplexCellModel(URI u,
	...
	this.attributes = attributes; // attributes are the same as the reference, but may have
									// different metadata
...
public static ComplexCellModel from(CellModel m) {
	...
	if (m.isComplex()) {
		...
		throw new IllegalArgumentException(
				"Tried to extract complex cell model from a simple one (" + m.getName() + "");
	}
