...
YAMLCellModelGuesserProcessor(String prefix, String case_, @Nullable String name, JsonNodeCellModel nodeCellmodel) {
...
public Context<JsonNodeCellModel, String> generateNewContext(Context<JsonNodeCellModel, String> oldContext) {
	...
	if (child==null) { 						// otherwise we check the children
		child = parent.children()
				.stream()
				.filter(c -> matches(c, node))
				.findAny()
				.orElse(null);
		//.orElseThrow(() -> new RuntimeException(new ParsingException("No child match for "+parent)));
	}
...
private boolean matches(CellModel candidate, JsonNode node) {

	boolean matches = false;
	boolean attributesOnly = candidate.getMetadata().getAttributesFor(case_).contains(Metadata.ATTRIBUTES_ONLY);

	if (candidate.isSimple()) {
	// first we see if we are in atributes only mode
		if (attributesOnly) {
		// if we have the attributes only it means we are complex
			throw new ClassCastException("Simple cell model "+candidate.getName()+" has "+Metadata.ATTRIBUTES_ONLY);
		}
	} else {
		ComplexCellModel complexCandidate = candidate.asComplex();
		if (attributesOnly) {
			matches = complexCandidate.attributes().stream().anyMatch(a -> node.has(matchName(a, case_)));
		} else {
			matches = complexCandidate.children().stream().anyMatch(c -> node.has(matchName(c, case_)));
		}
	}
